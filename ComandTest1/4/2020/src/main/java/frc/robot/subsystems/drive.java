...
public class drive extends Subsystem {
	...
	public Drivetrain() {
		...
		/*
		 * if (sixSim) { rightSlaveTwo.configFactoryDefault();
		 ...
		 * leftMaster.setSafetyEnabled(false); leftSlave.setSafetyEnabled(false); }
		 */
		...
		/*
		 * if (sixSim) { leftSlaveTwo.setNeutralMode(NeutralMode.Brake);
		...
		/*
		 * if (sixSim) { leftSlaveTwo.setInverted(false);
		...
		/*
		 * if (sixSim) { rightSlaveTwo.config_kP(0, P); rightSlaveTwo.config_kI(0, I);
		 * rightSlaveTwo.config_kD(0, D); rightSlaveTwo.config_kF(0, K,
		 * Presets.timeoutMS); // rightSlaveTwo.config_IntegralZone(0, IZ,
		 * Presets.timeoutMS); rightSlaveTwo.configClosedLoopPeakOutput(0, PO,
		 * Presets.timeoutMS); rightSlaveTwo.configAllowableClosedloopError(0, CLE,
		 * Presets.timeoutMS);
		 * 
		 * leftSlaveTwo.config_kP(0, P); leftSlaveTwo.config_kI(0, I);
		 * leftSlaveTwo.config_kD(0, D); leftSlaveTwo.config_kF(0, K,
		 * Presets.timeoutMS); // leftSlaveTwo.config_IntegralZone(0, IZ,
		 * Presets.timeoutMS); leftSlaveTwo.configClosedLoopPeakOutput(0, PO,
		 * Presets.timeoutMS); leftSlaveTwo.configAllowableClosedloopError(0, CLE,
		 * Presets.timeoutMS); }
		 */
		...
		/*
		 * if (sixSim) { rightSlaveTwo.follow(rightMaster);
		...
		Robot.networktable.table.getEntry("PIDReadout").setString(
				"P: " + P + "\tI: " + I + "\tD: " + D + "\tK: " + K + "\tIZ: " + IZ + "\tPO: " + PO + "\tCLE: " + CLE);
	}
	...
	public void periodic() {
		...
		Robot.networktable.table.getEntry("RawAccel").setDoubleArray(new double[] { RobotMap.accelerometer.getX(),
				RobotMap.accelerometer.getY(), RobotMap.accelerometer.getZ() });
		 ...
		 * (int)Robot.networktable.table.getEntry("changeCLE").getDouble(CLE) != CLE)
		 * PIDChange();
		 */
	}
	...
	public void tank(double left, double right, double threshhold) {
		if (Math.abs(leftMaster.getSelectedSensorVelocity()) < threshhold && Math.abs(left) > .75)
			left = Math.copySign(left * (((Math.abs(leftMaster.getSelectedSensorVelocity()) / threshhold) + .75) / 2),
					left);
		if (Math.abs(rightMaster.getSelectedSensorVelocity()) < threshhold && Math.abs(right) > .75)
			right = Math.copySign(
					right * (((Math.abs(rightMaster.getSelectedSensorVelocity()) / threshhold) + .75) / 2), right);
		tank(left, right);
	}
